...
@Controller
public class HistIncomeTaxController {
	
	@Autowired
	private HistIncomeTaxService histRegService;
	 ...
	 * @return
	 */
	@RequestMapping(value = "/HistoricalTaxReg")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/loadServerSideHistRegData")
	public String loadServerSideHistRegData(HttpServletResponse response, HttpServletRequest request)
	{
		/* getting the JSON response to load in data table*/
		String jsonResponse = histRegService.getDataTableResponse(request);
	...
	@RequestMapping("/downloadExcelHistRegData")
	public void downloadExcelHistRegData(HttpServletResponse response, HttpServletRequest request) throws IOException {
		System.out.println("Temp path:"+System.getProperty("java.io.tmpdir"));
		
		SXSSFWorkbook workBook = histRegService.getExcelData(request);
		//SXSSFWorkbook workBook = null;
		//HistIncomeTaxReportHandler HistIncTaxReportHandler = new HistIncomeTaxReportHandler();
		//workBook = HistIncTaxReportHandler.writeFile(result);
		response.setHeader("Content-Disposition",
				"attachment; filename=" + "Legacy_Federal_Tax_Asset_Register_Report_FA250.xlsx");
	...
	public Map<String,String> getCompanyGroups(){
		...
		List<String[]> list=histRegService.getCompanyGroups();
		for(String[] s:list) {
	...
	public Map<String,String> getInstances(){
		...
		List<String[]> list=histRegService.getInstances();
		for(String[] s:list) {
	...
	public Map<String,String> getYears(){
		...
		List<String[]> list=histRegService.getYears();
		for(String[] s:list) {
			yearMap.put(s[0], s[0]);
		}
		return yearMap;
	...
	@RequestMapping(value = "getReportNames", method = RequestMethod.GET)
	public @ResponseBody String getReportNames(
		...
		List<String[]> list=histRegService.getReportNames(reportList);
	...
	@RequestMapping(value = "getHistRegCompanyNames", method = RequestMethod.GET)
	public @ResponseBody String getCompanyNames(
	        @RequestParam(value = "groupName", required = true) String groupName) {
		Map<String, String> companyNamesMap = new TreeMap<String, String>();
		List<String[]> list=histRegService.getCompanyNames(groupName);
		for(String[] s:list) {
